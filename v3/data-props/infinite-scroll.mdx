...
Component dùng [Intersection Observer](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API) để phát hiện khi người dùng cuộn gần cuối nội dung và tự động kích hoạt request tải page tiếp theo. Dữ liệu mới được merge với nội dung hiện có thay vì thay thế.
...
Để biết thêm về tùy chọn reset, xem tài liệu [Resetting props](/v3/data-props/merging-props#resetting-props).
...
| `loadingPrevious` | Nội dung trước đó có đang được tải hay không   |
...
| `hasPrevious`     | Còn nội dung trước đó hay không                |
| `hasNext`         | Còn nội dung tiếp theo hay không               |
| `manualMode`      | Manual mode có đang active hay không           |
| `autoMode`        | Automatic loading có đang active hay không     |
...
Bài dịch này được đối chiếu từ [tài liệu Inertia.js v3 chính thức](https://inertiajs.com/docs/v3/data-props/infinite-scroll). Nếu có khác biệt do phiên bản hoặc cập nhật mới, hãy ưu tiên tài liệu chính thức làm nguồn tham chiếu.
