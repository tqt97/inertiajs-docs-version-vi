...
Một cách phổ biến để kiểm thử các page component JavaScript là dùng công cụ end-to-end như [Cypress](https://www.cypress.io/) hoặc [Laravel Dusk](https://laravel.com/docs/dusk). Đây là các công cụ tự động hóa trình duyệt cho phép chạy những mô phỏng thực tế của ứng dụng trong trình duyệt. Các test này thường chậm hơn; tuy nhiên, vì chúng kiểm thử ứng dụng ở đúng tầng mà người dùng cuối tương tác, chúng mang lại mức độ tin cậy cao rằng ứng dụng đang hoạt động chính xác. Và vì test chạy trong trình duyệt, code JavaScript cũng thực sự được thực thi và kiểm thử.

## Unit test phía client

Một cách khác để kiểm thử page component là dùng framework unit test phía client như [Jest](https://jestjs.io/) hoặc [Mocha](https://mochajs.org/). Cách tiếp cận này cho phép kiểm thử page component JavaScript độc lập bằng Node.js.
...
Hãy đi sâu vào các assertion hiện có. Trước tiên, để assert response Inertia có một property, bạn có thể dùng phương thức `has`. Có thể xem phương thức này tương tự hàm `isset` của PHP.
...
Bài dịch này được đối chiếu từ [tài liệu Inertia.js v1 chính thức](https://inertiajs.com/docs/v1/advanced/testing). Nếu có khác biệt do phiên bản hoặc cập nhật mới, hãy ưu tiên tài liệu chính thức làm nguồn tham chiếu.
