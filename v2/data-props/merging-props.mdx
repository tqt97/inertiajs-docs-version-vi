...
Inertia ghi đè các prop cùng tên khi tải lại một trang. Tuy nhiên, trong một số trường hợp bạn cần gộp dữ liệu mới với dữ liệu hiện có, chẳng hạn khi triển khai nút "tải thêm" cho kết quả phân trang. Component [Infinite scroll](/v2/data-props/infinite-scroll) sử dụng cơ chế gộp prop ở bên dưới.

Việc gộp prop chỉ hoạt động trong [partial reload](/v2/data-props/partial-reloads). Các lần truy cập tải đầy đủ trang luôn thay thế toàn bộ prop, kể cả khi bạn đã đánh dấu chúng để gộp.
...
Thay vì chỉ định những path lồng nhau cần gộp, bạn có thể dùng `Inertia::deepMerge()` để thực hiện deep merge cho toàn bộ cấu trúc.
...
Bạn cũng có thể gộp prop trực tiếp ở phía client mà không cần gửi request đến server bằng [client-side visits](/v2/the-basics/manual-visits#client-side-visits). Inertia cung cấp các [prop helper](/v2/the-basics/manual-visits#prop-helpers) để append, prepend hoặc thay thế giá trị prop.

## Kết hợp với deferred props

Bạn có thể kết hợp [deferred props](/v2/data-props/deferred-props) với các prop có thể gộp để trì hoãn việc tải prop và đánh dấu prop đó là có thể gộp sau khi được tải xong.
...
Tùy chọn request `reset` nhận một mảng chứa các key của prop mà bạn muốn reset.
...
Bài dịch này được đối chiếu từ [tài liệu Inertia.js v2 chính thức](https://inertiajs.com/docs/v2/data-props/merging-props). Nếu có khác biệt do phiên bản hoặc cập nhật mới, hãy ưu tiên tài liệu chính thức làm nguồn tham chiếu.
