...
Một số dữ liệu hiếm khi thay đổi, tốn chi phí tính toán hoặc đơn giản là rất lớn. Thay vì gửi dữ liệu này trong mọi response, bạn có thể dùng _once props_. Các prop này được client ghi nhớ và tái sử dụng trên các trang tiếp theo có cùng prop, đặc biệt phù hợp với [dữ liệu dùng chung](/v2/data-props/shared-data).
...
Bạn có thể refresh once prop từ phía client bằng [partial reload](/v2/data-props/partial-reloads). Máy chủ luôn resolve once prop khi prop được yêu cầu rõ ràng.
...
Once props tương thích với [prefetch](/v2/data-props/prefetching). Client tự động đưa mọi once prop đã ghi nhớ vào response prefetch, vì vậy khi điều hướng đến trang đã prefetch, once props đã có sẵn.

Trang prefetch chứa once prop đã hết hạn sẽ bị invalid khỏi cache.

## Kết hợp với loại prop khác

Modifier `once()` có thể chain vào prop [deferred](/v2/data-props/deferred-props), [merge](/v2/data-props/merging-props) và [optional](/v2/data-props/partial-reloads#lazy-data-evaluation).
...
Bài dịch này được đối chiếu từ [tài liệu Inertia.js v2 chính thức](https://inertiajs.com/docs/v2/data-props/once-props). Nếu có khác biệt do phiên bản hoặc cập nhật mới, hãy ưu tiên tài liệu chính thức làm nguồn tham chiếu.
