...
Ví dụ này sử dụng [form helper của Inertia](/v2/the-basics/forms#form-helper) để thuận tiện vì helper cho phép truy cập dễ dàng tiến trình upload hiện tại. Tuy nhiên, bạn vẫn có thể submit form bằng [Inertia visit thủ công](/v2/the-basics/manual-visits).
...
Tuy nhiên, một số framework như [Laravel](https://laravel.com/docs/routing#form-method-spoofing) và [Rails ](https://guides.rubyonrails.org/form_helpers.html#how-do-forms-with-patch-put-or-delete-methods-work-questionmark) hỗ trợ form method spoofing, cho phép tải file bằng `POST` nhưng để framework xử lý request như `PUT` hoặc `PATCH`. Cách này được thực hiện bằng cách thêm thuộc tính `_method` vào dữ liệu request.
...
Bài dịch này được đối chiếu từ [tài liệu Inertia.js v2 chính thức](https://inertiajs.com/docs/v2/the-basics/file-uploads). Nếu có khác biệt do phiên bản hoặc cập nhật mới, hãy ưu tiên tài liệu chính thức làm nguồn tham chiếu.
